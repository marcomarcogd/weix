...
Weix 从本机微信数据库读取新消息，通过大模型、规则或工作流生成回复，再调用微信客户端完成发送。项目同时提供 Vue 3 管理后台，用于选择微信账号、配置白名单、管理自动回复规则和查看发送结果。

当前版本以 **Windows 微信 4.x** 为主要维护与实测环境。Windows 默认使用 UI Automation（UIA）发送，优先尝试后台操作，不移动物理鼠标；macOS 保留 AppleScript 发送支持。
