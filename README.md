...
接入大模型，让 AI 替你自动回微信。此类自动化存在非零账号与合规风险，
建议先用非主账号、少量白名单和保守频率验证。

## 核心原理

- **收消息**：日常监听直接读取微信本地 SQLite 数据库；首次提取数据库密钥时，
  Windows 微信 4.1+ 会通过 `ReadProcessMemory` 只读访问微信进程
- **AI 回复**：LangChain 编排大模型，支持多轮对话、工具调用、意图识别
- **发消息**：两个平台均采用 GUI 模拟操作，不注入、不 Hook
...
| 维度 | Windows | macOS |
|------|---------|-------|
| 微信版本 | PC 微信 3.9.x；4.1.x 使用运行时 `Config.Cipher` 读取（版本更新可能失效） | Mac 微信 4.x (App Store) |
...
| 发送风险 | 非零（无注入，GUI 自动化仍可能被识别） | 非零（AppleScript 自动化仍可能被识别） |
| 管理员权限 | 需要 | 需要 |
...
> 以下措施只能降低风险，不能保证不会被检测、限制功能或封号。默认关闭自动回复，
> 开启前请先配置少量私聊/群聊白名单。密钥提取、客户端自动化也可能受微信协议约束。

1. **低侵入读取**：日常监听只读数据库；Windows 4.1+ 密钥提取只读进程内存，
   不写内存、不注入 DLL、不挂调试器，但仍有非零检测风险
2. **发消息零注入**：Windows / macOS 均采用 GUI 模拟操作，不注入 DLL、不 Hook 进程
